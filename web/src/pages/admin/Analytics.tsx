  ...
  return (
    <div style={{ paddingBottom: '2rem' }}>
      <div style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '2rem', fontWeight: 700, color: '#fff', marginBottom: '0.2rem' }}>Analytics</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '2rem' }}>
        ...
        <div className="admin-glass-card" style={{ display: 'flex', flexDirection: 'column' }}>
          <h3 style={{ fontSize: '1.2rem', color: '#fff', marginBottom: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '0.5rem' }}>Order Success Rate</h3>
            ...
            <div style={{ width: '50%', height: '250px' }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip contentStyle={{ background: '#1e0e1a', border: '1px solid rgba(160,32,240,0.3)', borderRadius: '8px' }} />
                </PieChart>
              </ResponsiveContainer>
            </div>
              ...
              {statusData.map(stat => (
                <div key={stat.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <div style={{ width: '12px', height: '12px', borderRadius: '50%', background: stat.color }}></div>
                    <span style={{ color: '#ccc', fontSize: '0.9rem' }}>{stat.name.replace('_', ' ')}</span>
                  </div>
                  <span style={{ fontWeight: 'bold', color: '#fff' }}>{stat.value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Top Riders */}
        <div className="admin-glass-card" style={{ display: 'flex', flexDirection: 'column' }}>
          <h3 style={{ fontSize: '1.2rem', color: '#fff', marginBottom: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '0.5rem' }}>Top Riders Leaderboard</h3>
          <div style={{ height: '250px', width: '100%' }}>
            {riderData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={riderData} layout="vertical" margin={{ top: 0, right: 0, left: 40, bottom: 0 }}>
                  <XAxis type="number" hide />
                  <YAxis type="category" dataKey="name" stroke="#a0a0a0" fontSize={12} axisLine={false} tickLine={false} />
                  <Tooltip cursor={{ fill: 'rgba(255,255,255,0.05)' }} contentStyle={{ background: '#1e0e1a', border: '1px solid rgba(160,32,240,0.3)', borderRadius: '8px' }} />
        ...
        {/* Historical Revenue */}
        <div className="admin-glass-card" style={{ display: 'flex', flexDirection: 'column' }}>
          <h3 style={{ fontSize: '1.2rem', color: '#fff', marginBottom: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '0.5rem' }}>Historical Revenue</h3>
          <div style={{ height: '250px', width: '100%' }}>
            {revenueData.length > 0 ? (
               <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={revenueData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <YAxis stroke="#a0a0a0" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(value) => `GH₵${value}`} />
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" vertical={false} />
                  <Tooltip 
                    contentStyle={{ background: '#1e0e1a', border: '1px solid rgba(160,32,240,0.3)', borderRadius: '8px' }}
        ...
        {/* Popular Zones */}
        <div className="admin-glass-card" style={{ display: 'flex', flexDirection: 'column' }}>
          <h3 style={{ fontSize: '1.2rem', color: '#fff', marginBottom: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '0.5rem' }}>Hottest Delivery Zones</h3>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {topLocations.length > 0 ? topLocations.map((loc, index) => (
              <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '1rem', background: 'rgba(255,255,255,0.02)', padding: '1rem', borderRadius: '12px' }}>
                <div style={{ width: '30px', height: '30px', borderRadius: '8px', background: index === 0 ? 'rgba(16, 185, 129, 0.2)' : 'rgba(160, 32, 240, 0.1)', color: index === 0 ? '#10b981' : 'var(--primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>
                  {index + 1}
                </div>
                <div style={{ flex: 1 }}>
                  <h4 style={{ color: '#fff', margin: 0, fontSize: '1rem' }}>{loc.name}</h4>
                  <div style={{ width: '100%', height: '4px', background: 'rgba(255,255,255,0.1)', borderRadius: '2px', marginTop: '6px' }}>
                    <div style={{ width: `${(loc.count / topLocations[0].count) * 100}%`, height: '100%', background: index === 0 ? '#10b981' : 'var(--primary)', borderRadius: '2px' }}></div>
                  </div>
                </div>
                <div style={{ fontWeight: 'bold', color: '#ccc', fontSize: '1.1rem' }}>
